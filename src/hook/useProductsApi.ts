...
import getItems from '@/lib/products/getItem';
...
export default function useProductsApi() {
    ...
    const { newProduct, setShowAddProduct, setNewProduct, setEditTagInput, setEditProduct, selectedProduct, setTagInput, setShowEditProduct, setSelectedProduct, setShowDeleteProduct, editProduct, image, setLoading, editImage, userId } = useProductStore()
    ...
    const { data: fetchedItems, isLoading: isFetchingItems } = useQuery<ProductItemsResponse, Error>({
        queryKey: ['get-items', userId],
        queryFn: () => getItems({ id: userId }),
        enabled: !!userId
    });
