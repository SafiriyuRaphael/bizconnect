...
export default function QueryErrorHandler({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const queryUnsubscribe = queryClient.getQueryCache().subscribe((event) => {
      if (event.type === 'updated' && event.query.state.error) {
        ...
        if (error.name === 'AuthRedirectError' && 'redirectTo' in error && error.redirectTo) {
    ...
    const mutationUnsubscribe = queryClient.getMutationCache().subscribe((event) => {
      if (event.type === 'updated' && event.mutation.state.status === 'error') {
        const error = event.mutation.state.error as AuthRedirectError | ApiError;
        if (error.name === 'AuthRedirectError' && 'redirectTo' in error && error.redirectTo) {
          router.push(error.redirectTo);
        } else {
          // Display non-redirect errors in MessageModal
